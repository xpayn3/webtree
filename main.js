...
const key = new THREE.DirectionalLight(0xfff4ea, 2.4); // near-daylight ~5500 K with a hint of warm
...
const fill = new THREE.DirectionalLight(0xc0d4ee, 1.1);
...
scene.add(rim);
// Soft ambient lift so the deepest shadow pockets read as form, not voids.
// 0.40 vs the old 0.12 trades a touch of contrast for a much prettier canopy
// (the old setup was photographer-harsh; this is closer to overcast-flattering).
...
const LIGHTING_PRESETS = {
  Majestic: {
    ...
    // for that look).
    key:     { color: 0xfff4ea, intensity: 2.4, pos: [-7, 13, -11] },
    fill:    { color: 0xc0d4ee, intensity: 1.1, pos: [8, 5, 11] },
    rim:     { color: 0xffffff, intensity: 0.8, pos: [11, 7, -3] },
...
// Singleton bark CanvasTextures — created ONCE, then `.image` is swapped to
// the canvas the generator produces for the active style. Because the
// THREE.Texture object identity stays stable across style changes, the TSL
// colorNode binding (built once when barkMat compiles) keeps working for the
// lifetime of the page. Live here (not in bark-textures.js) because they
// hold scene-resident GPU state, not pure CPU pixel data.
const barkAlbedo = (() => {
...
// material's colorNode is built — that way the TSL binding sees real pixels
// from frame zero. P doesn't exist yet at this point in the boot sequence;
// pass nothing for the third arg so the generator falls back to recipe
// defaults rather than crashing on the TDZ reference to `P`.
{
...
const LEAF_MAT_DEFAULTS = {
  // Vein normals visible at canopy distance + matte cuticle + thinner SSS
  // so backlit leaves glow against the backlight key without the plasticky
  // wax shine the previous defaults had. Higher roughness + lower sheen
  // reads as living leaf rather than enamelled prop.
  ...
  sheen: 0.1, sheenRoughness: 0.7, sheenColor: new THREE.Color(0xffffff),
};
...
function applyBarkStyle() {
  ...
  requestAnimationFrame(() => {
    ...
    const tex = generateBarkTexture(s, seed, P);
